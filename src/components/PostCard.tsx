import { Heart, MessageCircle, Share2, Lock, Eye, Crown, DollarSign, BadgeCheck, X, Play } from "lucide-react";
...
const PostCard = ({ creator, content, image, video, likes, comments, locked, type, price, timeAgo }: PostCardProps) => {
    ...
    <>
          ...
          <div className="relative cursor-pointer" onClick={() => !locked && (isVideo ? setVideoPlaying(true) : setFullscreen(true))}>
            {isVideo ? (
              <div className="relative" style={{ aspectRatio: "9/16", maxHeight: "500px" }}>
                <video
                  src={video}
                  className={`w-full h-full object-cover ${locked ? "blur-2xl scale-105" : ""}`}
                  muted
                  playsInline
                  loop
                />
              ...
              <img
                src={image}
                alt=""
                className={`w-full object-cover ${locked ? "blur-2xl scale-105" : ""}`}
      ...
      <AnimatePresence>
        {fullscreen && image && (
          ...
          >
            <button className="absolute top-4 right-4 z-10 text-white/70 hover:text-white" onClick={() => setFullscreen(false)}>
              <X className="w-6 h-6" />
            </button>
      ...
      <AnimatePresence>
        {videoPlaying && video && (
          ...
          >
            <button className="absolute top-4 right-4 z-10 text-white/70 hover:text-white" onClick={() => setVideoPlaying(false)}>
              <X className="w-6 h-6" />
            </button>
            <video
              src={video}
